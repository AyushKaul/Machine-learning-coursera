...
temp2 = Theta2;
temp2(:,1) = 0;   % because we don't add anything for j = 0

%disp(size(temp1));
%disp(size(temp2));
%J=J+((lambda/(2*m))*(sum(sum((temp1 .^ 2)))))+((lambda/(2*m))*(sum(sum((temp2 .^ 2)))));
J=J+((lambda/(2*m))*(sum(sum((temp1 .^ 2)))+sum(sum((temp2 .^ 2)))));

%Backpropagation

d3=a3-y_matrix;
d2=(d3*temp2(:,2:end)).*sigmoidGradient(z2);
delta1=d2'*a1;
delta2=d3'*a2;
